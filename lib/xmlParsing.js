...
var arrays = ['plans', 'customers', 'items', 'charges', 'invoices', 'subscriptions', 'transactions', 'promotions', 'coupons', 'incentives', 'errors'];
...
function validator(xpath, currentValue, newValue) {
    ...
    var arrayChild = newValue[child] ? [].concat(newValue[child]) : [];
...
function parseResult(data) {
    return new Promise(function (resolve, reject) {
        ...
        parser.parseString(data, function (err, xml) {
            ...
            var foundError = xml.error;

            if (!foundError && xml[type] && xml[type].errors && xml[type].errors.length) {
                var _xml$type$errors = _slicedToArray(xml[type].errors, 1);

                foundError = _xml$type$errors[0];
            }

            if (foundError) {
                var error = new Error(foundError._);
